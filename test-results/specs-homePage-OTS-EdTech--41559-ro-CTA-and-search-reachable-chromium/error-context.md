...
- Name: specs\homePage.spec.js >> OTS EdTech Homepage >> TC_HOME_025: Mobile viewport keeps hero CTA and search reachable
- Location: tests\specs\homePage.spec.js:631:3
...
Locator:  getByText('Explore', { exact: true })
Expected: visible
Received: hidden
Timeout:  15000ms
...
Call log:
  ...
  - waiting for getByText('Explore', { exact: true })
    19 × locator resolved to <p class="chakra-text css-1kwwu3d">Explore</p>
...
> 653 |       await expect(homePage.exploreNav).toBeVisible();
  ...
  717 |     });
  718 | 
  719 |     await allure.step('Open homepage', async () => {
  720 |       await homePage.goto(data.url);
  721 |     });
  722 |     await allure.step('Verify 100+ Courses badge', async () => {
  723 |       await expect(homePage.courseCountBadge).toBeVisible();
  724 |     });
  725 |   });
  726 | });
  727 | 
